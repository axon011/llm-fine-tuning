...
CELL_1_INSTALL = """!pip install -q transformers peft trl bitsandbytes datasets accelerate tiktoken kagglehub pandas
!nvidia-smi --query-gpu=name,memory.total --format=csv,noheader"""

CELL_2_DOWNLOAD = """import json, random, os
import kagglehub
import pandas as pd
...
CELL_3_PROCESS = """print("[2/6] Processing dataset...")
...
CELL_5_EVAL = """from peft import PeftModel
...
with open('data/test.jsonl') as f:
    for line in f:
        test_data_eval.append(json.loads(line))
...
for i, ex in enumerate(test_data_eval):
    ...
    try:
        idx = resp.find('{')
        parsed = json.loads(resp[idx:]) if idx >= 0 else json.loads(resp)
        results["json_valid"] += 1
        expected = json.loads(ex['output'])
        scores = {}
        for fld in STRING_FIELDS:
            p = str(parsed.get(fld, '')).lower().strip()
            e = str(expected.get(fld, '')).lower().strip()
            scores[fld] = 1.0 if p == e else (0.5 if e in p or p in e else 0.0)
        for fld in LIST_FIELDS:
            pl = {s.lower().strip() for s in parsed.get(fld, [])}
            el = {s.lower().strip() for s in expected.get(fld, [])}
            m = pl & el
            pr = len(m) / len(pl) if pl else 0
            rc = len(m) / len(el) if el else 0
            scores[fld] = {"f1": 2 * pr * rc / (pr + rc) if pr + rc else 0}
        results["field_scores"].append(scores)
    except Exception:
        results["json_invalid"] += 1
    print(f"  [{i+1}/{len(test_data_eval)}] valid: {results['json_valid']}/{results['json_valid']+results['json_invalid']}")

n = len(results["field_scores"])
if n:
    total_ex = results["json_valid"] + results["json_invalid"]
    print(f"\\nJSON validity: {results['json_valid']}/{total_ex}")
    print("\\nField accuracy:")
    for fld in STRING_FIELDS:
        avg = sum(s[fld] for s in results["field_scores"]) / n
        print(f"  {fld}: {avg:.2f}")
    for fld in LIST_FIELDS:
        avg_f1 = sum(s[fld]["f1"] for s in results["field_scores"]) / n
        print(f"  {fld}: F1={avg_f1:.2f}")
...
nb["cells"] = [
    md("# LLM Fine-Tuning: JD Structured Extractor\n\nQLoRA fine-tuning of Qwen2-0.5B on LinkedIn + hand-labeled JDs.\n\n**Runtime: T4 GPU required**"),
    code(CELL_1_INSTALL),
    code(CELL_2_DOWNLOAD),
    code(CELL_3_PROCESS),
    md("## Train with QLoRA"),
    code(CELL_4_TRAIN),
    md("## Evaluate on Test Set"),
    code(CELL_5_EVAL),
]

out_path = "C:/Users/Aravind/Desktop/workspace/GIT/llm-fine-tuning/notebooks/jd_extractor_colab.ipynb"
